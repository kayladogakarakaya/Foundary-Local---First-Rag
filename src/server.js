/**
 * Express server – AGORA, a local philosophy RAG assistant.
 * Serves the web UI and the /api/chat* endpoints (single-answer and the
 * per-philosopher stance-comparison mode). Fully offline; talks only to
 * the local Foundry Local runtime.
 */
...
// Last-resort safety net: this is a single-instance local server, so staying
// up to serve the next request is preferable to a hard crash. Real errors
// should still be caught and handled at their source (see chatEngine.js);
...
// ── API: Chat (compare stances across all authors, via SSE) ──
app.post("/api/chat/stances", async (req, res) => {
  try {
    const { message, compact } = req.body;
    if (!message || typeof message !== "string") {
      return res.status(400).json({ error: "message is required" });
    }
    ...
    res.write(`data: ${JSON.stringify({ type: "authors", data: engine.getAuthors() })}\n\n`);

    for await (const chunk of engine.queryStancesStream(message)) {
      res.write(`data: ${JSON.stringify(chunk)}\n\n`);
    }
    ...
    console.error("[API] Stances stream error:", err.message);
    res.write(`data: ${JSON.stringify({ type: "error", data: "Internal server error" })}\n\n`);
    res.end();
  }
});
...
app.post("/api/upload", express.raw({ type: "*/*", limit: "2mb" }), async (req, res) => {
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      store.insert(docId, title, category, i, chunks[i], author);
    }
...
async function start() {
  console.log("=== AGORA – Local Philosophy RAG Assistant ===\n");
